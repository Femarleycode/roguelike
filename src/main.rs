...
fn is_blocked(x: i32, y: i32, map: &Map, objects: &[Object]) -> bool {
    ...
    // noew check for any blocking object
    objects
        .iter()
...
fn get_names_under_mouse(mouse: Mouse, objects: &[Object], fov_map: &FovMap) -> String {
    ...
    names.join(", ") // join the names, seperates by commas
}
...
fn inventory_menu(inventory: &[Object], header: &str, root: &mut Root) -> Option<usize> {
    ...
    let option = if inventory.len() == 0 {
        vec!["Inventory is empty.".into()]
    } else {
        inventory.iter().map(|item| item.name.clone()).collect()
    };

    let inventory_index = menu(header, &option, INVENTORY_WIDTH, root);
...
fn main() {
    ...
    // a welcome message
    game.messages.add(
        "Welcome stranger! Prepare to enter the tomb of Thesius.",
        RED,
    );
