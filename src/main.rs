...
impl Messages {
    ...
    // add the new messages as tuple, with the text and the color
...
fn move_towards(id: usize, target_x: i32, target_y: i32, map: &Map, objects: &mut [Object]) {
    ...
    // normalise it to the length 1 (preserving direction), then round it and
...
// Mutably borrow two *seperate* element from the given slice.
...
// add to the player's invetory and move from the map
...
fn target_tile(
    ...
    loop {
        ...
        // accepts the target if the player clicked in FOC, and in case a range is
...
// returns a clicked monster FOV up to a range, or None if right-clicked
fn target_monster(
    tcod: &mut Tcod,
    game: &mut Game,
    objects: &[Object],
...
fn cast_fireball(
    ...
    // ask the player for a target tile to throw a fireball at game.messages.add(
    game.messages.add(
...
fn create_h_tunnel(x1: i32, x2: i32, y: i32, map: &mut Map) {
    // horizontal tunnel. "min()" and "max()" are used in case "x1 > x21"
...
fn make_map(objects: &mut Vec<Object>) -> Map {
    ...
    // --only works when the player is the first object
    ...
    for _ in 0..MAX_ROOMS {
        ...
        if !failed {
            ...
            // add content to this room, such as monsters
            ...
            if rooms.is_empty() {
                ...
                // connect to a previous room with a tunnel
...
fn level_up(tcod: &mut Tcod, game: &mut Game, objects: &mut [Object]) {
    ...
    let level_up_xp = LEVEL_UP_BASE + player.level * LEVEL_UP_FACTOR;
    // see if exp is enough to level up
    if player.fighter.as_ref().map_or(0, |f| f.xp) >= level_up_xp {
        // it is, so level up
        player.level += 1;
        game.messages.add(
            format!(
                "Your skills grow stronger! You reached level {}!",
                player.level
            ),
            YELLOW,
        );
...
fn monster_death(monster: &mut Object, game: &mut Game) {
    // transform it into a corpse which doesn't block, can't be
...
fn play_game(tcod: &mut Tcod, game: &mut Game, objects: &mut Vec<Object>) {
    ...
    while !tcod.root.window_closed() {
        ...
        if player_action == PlayerAction::Exit {
            save_game(game, objects).unwrap(); // find new home for this
            break;
        }
